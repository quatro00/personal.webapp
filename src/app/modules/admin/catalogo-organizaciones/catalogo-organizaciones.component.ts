...
@Component({
  selector: 'app-catalogo-organizaciones',
  standalone: true,
  templateUrl: './catalogo-organizaciones.component.html',
  imports: [
    CommonModule,
    MatTableModule,
    MatSortModule,
...
export class CatalogoOrganizacionesComponent {
  ...
  activar(item){
    this.admin_organizacionService.Reactivar(item.id)
    .subscribe({
      next: (response) => {
        this.loadData();
      },
  ...
  desactivar(item){
    this.admin_organizacionService.Desactivar(item.id)
    .subscribe({
      next: (response) => {
        this.loadData();
      },
